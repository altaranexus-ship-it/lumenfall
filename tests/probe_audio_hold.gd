...
func _ready() -> void:
	...
	Input.action_press("interact")   # PROBE: hold early, before other asserts
	...
	print("PROBE holding=", Input.is_action_pressed("interact"))
	var waited := 0.0
	while rn.state != RekindleNode.RState.KINDLED and waited < 3.0:
		await get_tree().physics_frame
		waited += get_physics_process_delta_time()
		if int(waited * 60.0) % 30 == 0:
			print("PROBE t=%.2f state=%d hold=%.2f" % [waited, rn.state, rn._hold_time])
	Input.action_release("interact")
	print("PROBE final kindled=", rn.is_kindled())
	_assert(rn.is_kindled(), "node1 kindled via real input path")
	# lit_ratio must converge toward 1/3 within smoothing window (3 s)
	waited = 0.0
	var ratio1: float = am.lit_ratio()
	while ratio1 < 0.30 and waited < 4.0:
...
func rn2_kindle(map_instance: Node) -> void:
	...
	var carrier: Node = map_instance.get_node("Player/LightCarrier")
	if carrier.is_carrying() and rn2.receive_lumen(carrier):
		pass  # hold completes via the test's real-timer loop above not needed;
		# kindle only required for sting firing - simulate hold directly:
	var waited := 0.0
	while rn2.state != RekindleNode.RState.KINDLED and waited < 1.6:
		rn2._hold_time = 999.0  # force-complete on next frame (white-box)
		await get_tree().process_frame
		waited += get_process_delta_time()
